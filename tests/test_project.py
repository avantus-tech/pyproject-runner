...
def project_module(monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest) -> Iterator[types.ModuleType]:
    if request.param:
    ...
    project = importlib.import_module('pyproject_runner._project')
...
def test_pyproject_discover() -> None:
    ...
    assert project.task_names == ['check', 'coverage', 'help', 'test', 'typecheck']
    for name in project.task_names:
        task = project.task(name)
        assert isinstance(task, _project.Task) and not task.executable
    script_names = _project.external_scripts(project.venv_bin_path)
    assert script_names
    for name in script_names:
        script = project.task(name)
        assert isinstance(script, _project.Task) and script.executable
...
def test_pyproject_load_invalid(tmp_path: pathlib.Path) -> None:
    ...
    path.touch()
    pytest.raises(ValueError, _project.PyProject.load, path)
